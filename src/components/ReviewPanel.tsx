 ...
 * or mastered, plus a 7-day forecast of upcoming reviews. Hidden until the user
 * has started reviewing anything.
 */
export function ReviewPanel() {
  ...
  if (learning.length === 0 && mastered === 0) return null;
  ...
  return (
    <section className="mb-8">
      <div className="flex items-center justify-between mb-3">
        ...
        >
          {dueNow ? `Review ${dueNow} due →` : 'All caught up ✓'}
        </Link>
      </div>

      <div className="rounded-xl border border-border bg-bg-card p-4">
        <div className="grid grid-cols-3 gap-2 mb-4">
          <Stat value={dueNow} label="Due now" color="text-accent-orange" />
          <Stat value={learning.length} label="Learning" color="text-accent-cyan" />
          <Stat value={mastered} label="Mastered" color="text-accent-green" />
        </div>
