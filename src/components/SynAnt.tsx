        ...
        <div>
          ...
          <div className="flex flex-wrap gap-1.5">
            {wordData.synonyms!.map((s) => (
              <span key={s} className="text-xs px-2.5 py-1 rounded-full bg-accent-cyan/10 text-accent-cyan border border-accent-cyan/20">{mask(s)}</span>
            ))}
          </div>
        </div>
      )}
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-1.5">
            {wordData.antonyms!.map((a) => (
              <span key={a} className="text-xs px-2.5 py-1 rounded-full bg-accent-red/10 text-accent-red border border-accent-red/20">{mask(a)}</span>
            ))}
          </div>
        </div>
      )}
